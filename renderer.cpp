...
ID3D11Buffer* Renderer::m_EnemyBuffer = NULL;
...
void Renderer::Init()
{
	...
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_MIRROR;//WARP
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_MIRROR;//WARP
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_MIRROR;//WARP
	...
	bufferDesc.ByteWidth = sizeof(ENEMYPARAM);

	m_Device->CreateBuffer(&bufferDesc, NULL, &m_EnemyBuffer);
	m_DeviceContext->VSSetConstantBuffers(7, 1, &m_EnemyBuffer);
	m_DeviceContext->PSSetConstantBuffers(7, 1, &m_EnemyBuffer);
	...
	//シャドウバッファ作成
	{
		...
		textureDesc.Width = 10000;
		textureDesc.Height = 10000;
		textureDesc.MipLevels = 1;
		textureDesc.ArraySize = 1;
...
void Renderer::SetEnemy(ENEMYPARAM Enemy)
{
	m_DeviceContext->UpdateSubresource(m_EnemyBuffer, 0, NULL, &Enemy, 0, 0);
}
...
void Renderer::SetDepthViewport(void)
{
	// ビューポート設定
	D3D11_VIEWPORT vp;
	vp.Width = (FLOAT)10000;
	vp.Height = (FLOAT)10000;
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
	vp.TopLeftX = 0;
	vp.TopLeftY = 0;
...
void Renderer::SetRasterizerEnable(bool flg)
{
	if (flg)
